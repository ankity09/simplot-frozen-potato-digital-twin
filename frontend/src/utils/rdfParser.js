...
export class RDFParser {
  ...
  isSchemaNode(uri) {
    ...
    if (uri.includes('www.w3.org/2000/01/rdf-schema') ||
        uri.includes('www.w3.org/1999/02/22-rdf-syntax-ns')) {
      return true;
    }
    ...
    return uri.includes('digitaltwin/') && (
      ...
      uri.endsWith('/Sensor') ||
      uri.endsWith('/ProductionLine') ||
      uri.endsWith('/ProcessingStage') ||
      uri.endsWith('/inLine') ||
      uri.endsWith('/partOf') ||
      uri.endsWith('/dependsOn') ||
      uri.endsWith('/propagates') ||
      uri.endsWith('/componentOf') ||
      uri.endsWith('/upstream')
    );
  }
  ...
  isInstanceNode(uri) {
    ...
    return uri.includes('example.com/factory/') ||
           ...
           uri.includes('example.com/potato-factory/');  // ADD POTATO FACTORY NAMESPACE
  }
  ...
  getNodeType(uri) {
    ...
    // Potato factory types
    if (type.includes('ProductionLine')) return 'line';
    if (type.includes('ProcessingStage')) return 'stage';
  ...
  isRelationshipPredicate(predicate) {
    const relationshipPredicates = [
      ...
      // Potato factory relationships
      'http://example.com/potato-factory/inLine',
      'http://example.com/potato-factory/componentOf',
      'http://example.com/potato-factory/upstream',
      'http://example.com/potato-factory/dependsOn',
